...
Luckily, we have build a nice diagram that shows us which step of patient care each module corresponds to:
...
MIMIC-ICU is basically the same critical care data that was present on MIMIC-III, but without *carevue*. Here's a summary:
...
- inputevents: only one table now, for *metavision*.
